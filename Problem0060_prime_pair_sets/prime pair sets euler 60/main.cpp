...
{  clock_t r=clock();
 ...
 uint64_t babelNum[3];
    ...
    list<set <uint64_t> >::iterator ppp;
    set <uint64_t>::iterator mmm;
        for (ppp=fox.begin();ppp!=fox.end();ppp++){
            int total=0;
            for (mmm=(*ppp).begin();mmm!=(*ppp).end();mmm++){
                total+=(*mmm);
    ...
    cout<<best<<" is the sum of these five primes: ";
    for (mmm=dog.begin();mmm!=dog.end();mmm++){
        cout<<"  "<<*mmm;
    }
    cout<<endl;
